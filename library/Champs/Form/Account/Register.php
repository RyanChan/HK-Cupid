...
class Champs_Form_Account_Register extends Champs_FormProcessor {
    ...
    public function process(\Zend_Controller_Request_Abstract $request) {
        ...
        if (!$this->hasError()) {
            try {
                $user = $this->_userRepository->storeUserEntity($this->user);
                // set first name profile
                $user->setProfileWithKeyAndValue('first_name', $this->first_name);
                // set last name profile
                $user->setProfileWithKeyAndValue('last_name', $this->last_name);
                // set email profile
                $user->setProfileWithKeyAndValue('email', $this->email);
                // update the profile
                $this->em->flush();
